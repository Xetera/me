...
const trace = otlp.trace.getTracer("me.spotify");

const spotifyLikedProvider = makeProvider({
  name: "spotify.liked",
  schedule: cronJob("0 */12 * * *"),
  async run({ config, prisma }) {
    return trace.startActiveSpan("spotify.run", async (span) => {
  ...
  async queryLatest({ prisma }) {
    return trace.startActiveSpan("spotify.queryLatest", async (span) => {
